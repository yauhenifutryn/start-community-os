...
Current status: the current event's four-export aggregate, rich GitHub evidence, reviewed semantic projection, and enriched partner HTML/PDF generator are implemented. The exact current HTML/PDF share approval and any external deployment remain pending. GitHub, bounded applicant-public-page, optional Coresignal evaluation, and OpenAI semantic processing remain behind separate machine gates. Coresignal is not a partner-report prerequisite or release input. Privacy-minimized PostHog wiring is an explicit local pre-deployment step and is off by default. No deployment or external publication is performed by the test suite.
