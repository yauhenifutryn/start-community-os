...
5. In the local release-owner operator, choose "Prepare analytics bundle". The operator reads the live EU project settings, verifies `anonymize_ips=true`, verifies that the project returns the same public key, writes a protected hash receipt, and binds that receipt to the analytics approval and deployment manifest. A checkbox or verbal confirmation is not accepted.
